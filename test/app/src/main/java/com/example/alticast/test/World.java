...
//	private BufferedImage shownHealthBar; //represents parts of the health bar picture based on the current health of the planet
	...
	public World(int w, int h,ArrayList<String> ArrayWord){
...
*/
		...
		String word_temp = wordList.get((int)(Math.random()*wordList.size()));
		line1 = new Line_Opponents (0, 0, 0, 0,level,word_temp);
		word_temp = wordList.get((int)(Math.random()*wordList.size()));
		line2 = new Line_Opponents (30, 0, 30, 0,level,word_temp);
		word_temp = wordList.get((int)(Math.random()*wordList.size()));
		line3 = new Line_Opponents (200, 0, 200, 0,level,word_temp);
		word_temp = wordList.get((int)(Math.random()*wordList.size()));
		line4 = new Line_Opponents (550, 0, 550, 0,level,word_temp);
	...
	public void updateEnemyMissiles(double planetSurfaceSize) { //this method controls all of the attributes of the enemy line missiles
		...
		for ( int x = 0 ; x < oppo.size () ; x++)//goes through line opponents array again
		{
			...
			if (tempObj.getYCoor () >= height - planetSurfaceSize)//if the line hits the surface of the planet then...
			{
				addExplosions(tempObj.getXCoor(), tempObj.getYCoor(), 0.1, 0.5);//add the explosion there
				...
				if(level==1){//if the level the user is playing at is level 1 then
					...
					if (oppo.size() < 4){//if the number of lines in the vector is less than 4 then
						...
						String word_temp = wordList.get((int)(Math.random()*wordList.size()));
						Line_Opponents line5 = new Line_Opponents(tempCoor,0,tempCoor,0,level,word_temp);//new line declared using arbitrary line opponents object
					...
					if (oppo.size() < 4){
						...
						String word_temp = wordList.get((int)(Math.random()*wordList.size()));
						Line_Opponents line5 = new Line_Opponents(tempCoor,0,tempCoor,0,level,word_temp);
						oppo.add(line5);
					}
				}
				else if(level==3){
